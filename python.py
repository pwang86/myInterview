...
class Solution:
    ...
    def findItinerary(self, tickets: List[List[str]]) -> List[str]:
        ...
        res.append("JFK")
        for t in tickets:
            myMap[t[0]].append(t[1])
        for m in myMap.values():
            m.sort()
        def backtrack(start: str) -> bool:
            ...
            if start not in myMap:
                return False
            if len(myMap[start]) == 0:
                return False
            for i in range(len(myMap[start])):
                city = myMap[start][i]
                myMap[start].pop(i)
                res.append(city)
                if backtrack(city):
